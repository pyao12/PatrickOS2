...
extern "C" void _start(const program_api_t *api) {
    char source[256];
    char target[256];
    if (api->argument == 0 || !split_two_args(api->argument, source, target, sizeof(source))) {
        api->write_console("mv: usage mv <src> <dst>\n", 0xff0000);
        return;
    }

    char source_path[256];
    char target_path[256];
    resolve_path(api->cwd, source, source_path, sizeof(source_path));
    resolve_path(api->cwd, target, target_path, sizeof(target_path));
    if (api->rename_path(source_path, target_path) < 0) api->write_console("mv: rename failed\n", 0xff0000);
}
