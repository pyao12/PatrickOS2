...
static void copy_bytes(ui8 *dst, const ui8 *src, ui32 size) {
    for (ui32 index = 0; index < size; index++) dst[index] = src[index];
}

static void set_bytes(ui8 *dst, ui8 value, ui32 size) {
    for (ui32 index = 0; index < size; index++) dst[index] = value;
}
...
static bool fat32_mount_writable(const fat32_filesystem_t *filesystem) {
    return filesystem != 0 && filesystem->mounted && filesystem->read_sector != 0 &&
...
static bool fat32_read_fat_entry(const fat32_filesystem_t *filesystem, ui32 cluster, ui32 *value) {
    if (!fat32_cluster_valid(filesystem, cluster) || value == 0) return false;

    ui32 entries_per_sector = fat32_sector_size / sizeof(ui32);
    ui32 fat_sector = cluster / entries_per_sector;
    if (fat_sector >= filesystem->sectors_per_fat) return false;

    ui8 sector[fat32_sector_size];
    if (!fat32_read_sector(filesystem, filesystem->fat_start_lba + fat_sector, sector)) return false;
...
static ui32 fat32_next_cluster(const fat32_filesystem_t *filesystem, ui32 cluster) {
    ui32 value = 0;
    if (!fat32_read_fat_entry(filesystem, cluster, &value)) return 0;
    return value;
}
...
static bool fat32_write_fat_entry(const fat32_filesystem_t *filesystem, ui32 cluster, ui32 value) {
    ...
    for (ui32 fat_index = 0; fat_index < filesystem->fat_count; fat_index++) {
        ...
        ui8 sector[fat32_sector_size];
        if (!fat32_read_sector(filesystem, lba, sector)) return false;
        ui32 existing = read_le32(sector + offset) & 0xf0000000;
        write_le32(sector + offset, existing | (value & 0x0fffffff));
        if (!fat32_write_sector(filesystem, lba, sector)) return false;
    }
...
static bool fat32_zero_cluster(const fat32_filesystem_t *filesystem, ui32 cluster) {
    ...
    for (ui32 sector_index = 0; sector_index < filesystem->sectors_per_cluster; sector_index++) {
        if (!fat32_write_sector(filesystem, cluster_lba + sector_index, sector)) return false;
    }
    return true;
}

static bool fat32_allocate_cluster(const fat32_filesystem_t *filesystem, ui32 *cluster_out) {
    if (cluster_out == 0) return false;

    for (ui32 cluster = 2; cluster <= filesystem->cluster_count + 1; cluster++) {
        ui32 value = 0;
        if (!fat32_read_fat_entry(filesystem, cluster, &value)) return false;
        if (value != 0) continue;
        if (!fat32_write_fat_entry(filesystem, cluster, fat32_end_of_chain) ||
            !fat32_zero_cluster(filesystem, cluster)) {
            return false;
        }
        *cluster_out = cluster;
        return true;
...
static bool fat32_free_cluster_chain(const fat32_filesystem_t *filesystem, ui32 first_cluster) {
    ...
    while (fat32_cluster_valid(filesystem, cluster)) {
        ...
        if (!fat32_write_fat_entry(filesystem, cluster, 0)) return false;
        if (fat32_cluster_end(next_cluster)) return true;
...
static bool fat32_make_name(const char *start, ui32 length, ui8 name[11]) {
    ...
    for (ui32 index = 0; index < length; index++) {
        ...
        if (character < 0x21 || character == '"' || character == '*' || character == '+' ||
    ...
    if (base_length == 0 || base_length > 8 || extension_length > 3 ||
        ...
        (base_length == 1 && start[0] == '.') ||
        (base_length == 2 && start[0] == '.' && start[1] == '.')) {
        return false;
    }
...
static ui32 fat32_entry_first_cluster(const ui8 *entry) {
    return ((ui32) read_le16(entry + 20) << 16) | read_le16(entry + 26);
}
...
static void fat32_build_entry(ui8 *entry, const ui8 name[11], ui8 attributes,
...
static bool fat32_write_entry_at(const fat32_filesystem_t *filesystem,
                                 const fat32_entry_location_t &location, const ui8 entry[32]) {
    ui8 sector[fat32_sector_size];
    if (!fat32_read_sector(filesystem, location.sector_lba, sector)) return false;
    copy_bytes(sector + location.offset, entry, 32);
    return fat32_write_sector(filesystem, location.sector_lba, sector);
}

static bool fat32_find_in_directory(const fat32_filesystem_t *filesystem, ui32 directory_cluster,
                                    const ui8 name[11], fat32_entry_location_t *location) {
    ...
    for (ui32 cluster_index = 0; cluster_index < filesystem->cluster_count; cluster_index++) {
        ...
        for (ui32 sector_index = 0; sector_index < filesystem->sectors_per_cluster; sector_index++) {
            ...
            if (!fat32_read_sector(filesystem, sector_lba, sector)) return false;
...
static bool fat32_find_free_directory_slot(const fat32_filesystem_t *filesystem,
    ...
    while (true) {
        ...
        for (ui32 sector_index = 0; sector_index < filesystem->sectors_per_cluster; sector_index++) {
            ...
            for (ui32 offset = 0; offset < fat32_sector_size; offset += 32) {
                if (sector[offset] != 0x00 && sector[offset] != 0xe5) continue;
                location->cluster = cluster;
                location->sector_lba = sector_lba;
                location->offset = offset;
        ...
        if (fat32_cluster_end(next_cluster)) {
            ...
            if (!fat32_allocate_cluster(filesystem, &new_cluster) ||
                !fat32_write_fat_entry(filesystem, cluster, new_cluster) ||
...
static bool fat32_resolve_parent(const fat32_filesystem_t *filesystem, const char *path,
                                 ui32 *parent_cluster, ui8 name[11],
                                 fat32_entry_location_t *existing_location, bool *exists) {
    if (!fat32_mount_writable(filesystem) || path == 0 || parent_cluster == 0 || name == 0 ||
        exists == 0) {
        return false;
    }
    ...
    while (true) {
        ...
        ui8 segment_name[11];
        if (!fat32_make_name(segment_start, segment_length, segment_name)) return false;
        ...
        if (final_segment) {
            ...
            *exists = fat32_find_in_directory(filesystem, directory_cluster, segment_name,
                                              existing_location);
            return true;
        }
        ...
        if (!fat32_find_in_directory(filesystem, directory_cluster, segment_name, &child) ||
            (child.entry[11] & fat32_attribute_directory) == 0) {
            return false;
        }

        ui32 child_cluster = fat32_entry_first_cluster(child.entry);
        if (!fat32_cluster_valid(filesystem, child_cluster)) return false;
        directory_cluster = child_cluster;
    }
}

static bool fat32_update_entry_size_and_cluster(const fat32_filesystem_t *filesystem,
                                                fat32_entry_location_t *location,
                                                ui32 first_cluster, ui32 size) {
    if (location == 0) return false;
...
static bool fat32_directory_parent_cluster(const fat32_filesystem_t *filesystem, ui32 cluster,
                                           ui32 *parent_cluster) {
    ...
    ui8 sector[fat32_sector_size];
    if (!fat32_read_sector(filesystem, cluster_lba, sector)) return false;
...
static bool fat32_directory_is_descendant(const fat32_filesystem_t *filesystem,
                                          ui32 directory_cluster, ui32 ancestor_cluster) {
    ui32 cluster = directory_cluster;
    for (ui32 depth = 0; depth < filesystem->cluster_count; depth++) {
        if (cluster == ancestor_cluster) return true;
        if (cluster == filesystem->root_cluster) return false;
        if (!fat32_directory_parent_cluster(filesystem, cluster, &cluster)) return false;
    }
    return false;
}
...
static bool fat32_directory_empty(const fat32_filesystem_t *filesystem, ui32 cluster) {
    for (ui32 cluster_index = 0; cluster_index < filesystem->cluster_count; cluster_index++) {
        ...
        for (ui32 sector_index = 0; sector_index < filesystem->sectors_per_cluster; sector_index++) {
            ui8 sector[fat32_sector_size];
            if (!fat32_read_sector(filesystem, cluster_lba + sector_index, sector)) return false;
            for (ui32 offset = 0; offset < fat32_sector_size; offset += 32) {
                const ui8 *entry = sector + offset;
                if (entry[0] == 0x00) return true;
                if (entry[0] == 0xe5 || entry[11] == fat32_attribute_long_name ||
                    (entry[11] & 0x08) != 0 || fat32_directory_entry_is_dot(entry)) {
                    continue;
                }
                return false;
            }
...
static bool fat32_nth_cluster(const fat32_filesystem_t *filesystem, ui32 first_cluster,
                              ui32 index, ui32 *cluster_out) {
    if (cluster_out == 0) return false;
    ui32 cluster = first_cluster;
...
static bool fat32_chain_length(const fat32_filesystem_t *filesystem, ui32 first_cluster,
                               ui32 *length, ui32 *last_cluster) {
    if (length == 0 || last_cluster == 0) return false;
    if (first_cluster == 0) {
        *length = 0;
        *last_cluster = 0;
        return true;
...
static bool fat32_resize_chain(const fat32_filesystem_t *filesystem, ui32 *first_cluster,
                               ui32 cluster_count) {
    if (first_cluster == 0) return false;

    if (cluster_count == 0) {
        bool ok = *first_cluster == 0 || fat32_free_cluster_chain(filesystem, *first_cluster);
        if (ok) *first_cluster = 0;
        return ok;
    }

    ui32 current_length = 0;
    ui32 last_cluster = 0;
    if (!fat32_chain_length(filesystem, *first_cluster, &current_length, &last_cluster)) return false;

    if (current_length == 0) {
        if (!fat32_allocate_cluster(filesystem, first_cluster)) return false;
        current_length = 1;
    ...
    if (current_length == cluster_count) return true;

    ui32 keep_last = 0;
    if (!fat32_nth_cluster(filesystem, *first_cluster, cluster_count - 1, &keep_last)) return false;

    ui32 tail = fat32_next_cluster(filesystem, keep_last);
    if (!fat32_write_fat_entry(filesystem, keep_last, fat32_end_of_chain)) return false;
    if (fat32_cluster_valid(filesystem, tail)) return fat32_free_cluster_chain(filesystem, tail);
    return fat32_cluster_end(tail);
}

static bool fat32_write_range(const fat32_filesystem_t *filesystem, ui32 first_cluster,
                              ui32 offset, const ui8 *buffer, ui32 size, bool zero_fill) {
    ...
    while (bytes_written < size) {
        ...
        while (sector_index < filesystem->sectors_per_cluster && bytes_written < size) {
            ...
            if (!fat32_write_sector(filesystem, sector_lba, sector)) return false;
            bytes_written += chunk;
            sector_index++;
            sector_offset = 0;
        }
...
static bool fat32_mark_entry_deleted(const fat32_filesystem_t *filesystem,
                                     const fat32_entry_location_t &location) {
    ui8 entry[32];
...
static bool fat32_write_dot_entries(const fat32_filesystem_t *filesystem, ui32 cluster,
                                    ui32 parent_cluster) {
    ui32 cluster_lba;
    if (!fat32_cluster_lba(filesystem, cluster, &cluster_lba)) return false;
...
bool fat32_create_file(fat32_filesystem_t *filesystem, const char *path) {
    ...
    bool exists = false;
    if (!fat32_resolve_parent(filesystem, path, &parent_cluster, name, &existing, &exists) || exists) {
        return false;
    }

    fat32_entry_location_t slot;
    if (!fat32_find_free_directory_slot(filesystem, parent_cluster, &slot)) return false;
...
bool fat32_create_directory(fat32_filesystem_t *filesystem, const char *path) {
    ...
    ui32 cluster = 0;
    if (!fat32_allocate_cluster(filesystem, &cluster) ||
        !fat32_write_dot_entries(filesystem, cluster, parent_cluster)) {
        if (cluster != 0) fat32_free_cluster_chain(filesystem, cluster);
        return false;
    }
...
i64 fat32_write(fat32_filesystem_t *filesystem, const char *path, ui32 offset, const ui8 *buffer,
                ui32 size) {
    ...
    if (!fat32_resolve_parent(filesystem, path, &parent_cluster, name, &location, &exists) || !exists ||
        (location.entry[11] & fat32_attribute_directory) != 0) {
        return fat32_write_error;
    }
    ...
    if (end_offset > 0xffffffffULL) return fat32_write_error;

    ui32 new_size = file_size;
    if (end_offset > file_size) new_size = (ui32) end_offset;

    ui32 cluster_size = (ui32) filesystem->sectors_per_cluster * fat32_sector_size;
    ui32 required_clusters = new_size == 0 ? 0 : (new_size + cluster_size - 1) / cluster_size;
    if (!fat32_resize_chain(filesystem, &first_cluster, required_clusters)) return fat32_write_error;
...
bool fat32_rename(fat32_filesystem_t *filesystem, const char *path, const char *new_path) {
    ...
    if (!fat32_resolve_parent(filesystem, path, &source_parent_cluster, source_name, &source,
    ...
    if (!fat32_resolve_parent(filesystem, new_path, &target_parent_cluster, target_name, &target,
    ...
    fat32_entry_location_t slot;
    if (!fat32_find_free_directory_slot(filesystem, target_parent_cluster, &slot)) return false;

    ui8 moved_entry[32];
    copy_bytes(moved_entry, source.entry, sizeof(moved_entry));
    copy_bytes(moved_entry, target_name, 11);
    if (!fat32_write_entry_at(filesystem, slot, moved_entry)) return false;
...
bool fat32_remove(fat32_filesystem_t *filesystem, const char *path) {
    ...
    if (first_cluster != 0 && !fat32_free_cluster_chain(filesystem, first_cluster)) return false;
