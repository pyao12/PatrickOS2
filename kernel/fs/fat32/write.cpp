...
static bool fat32_mount_writable(const fat32_filesystem_t *filesystem) {
    return filesystem != 0 && filesystem->mounted &&
           filesystem->read_sector != 0 && filesystem->write_sector != 0;
}

static bool fat32_read_sector(const fat32_filesystem_t *filesystem, ui32 lba,
                              ui8 *buffer) {
    return filesystem->read_sector(filesystem->read_context, lba, buffer);
}

static bool fat32_write_sector(const fat32_filesystem_t *filesystem, ui32 lba,
                               const ui8 *buffer) {
...
static bool fat32_write_fat_entry(const fat32_filesystem_t *filesystem,
    ...
    for (ui32 fat_index = 0; fat_index < filesystem->fat_count; fat_index++) {
        ui32 lba = filesystem->fat_start_lba +
...
static bool fat32_zero_cluster(const fat32_filesystem_t *filesystem,
                               ui32                      cluster) {
    ui32 cluster_lba;
    ...
    for (ui32 sector_index = 0; sector_index < filesystem->sectors_per_cluster;
         sector_index++) {
...
static bool fat32_allocate_cluster(const fat32_filesystem_t *filesystem,
    ...
    for (ui32 cluster = 2; cluster <= filesystem->cluster_count + 1;
         cluster++) {
        ui32 value = 0;
        if (!fat32_read_fat_entry(filesystem, cluster, &value))
            return false;
...
static bool fat32_free_cluster_chain(const fat32_filesystem_t *filesystem,
                                     ui32                      first_cluster) {
        ...
        for (ui32 sector_index = 0;
            ...
            for (ui32 offset = 0; offset < fat32_sector_size; offset += 32) {
                ...
                if (directory_entry[0] == 0xe5 ||
                    directory_entry[11] == fat32_attribute_long_name ||
...
static bool fat32_resolve_parent(const fat32_filesystem_t *filesystem,
                                 const char *path, ui32 *parent_cluster,
                                 ui8                     name[11],
                                 fat32_entry_location_t *existing_location,
                                 bool                   *exists) {
    ...
    while (true) {
        ...
        if (final_segment) {
            ...
            *exists = fat32_find_in_directory(filesystem, directory_cluster,
                                              segment_name, existing_location);
            return true;
        }
...
static bool
fat32_update_entry_size_and_cluster(const fat32_filesystem_t *filesystem,
                                    fat32_entry_location_t   *location,
                                    ui32 first_cluster, ui32 size) {
    if (location == 0) {
        ;
...
static bool fat32_directory_empty(const fat32_filesystem_t *filesystem,
                                  ui32                      cluster) {
    for (ui32 cluster_index = 0; cluster_index < filesystem->cluster_count;
         cluster_index++) {
        ui32 cluster_lba;
        ...
        for (ui32 sector_index = 0;
            ...
            for (ui32 offset = 0; offset < fat32_sector_size; offset += 32) {
                ...
                if (entry[0] == 0x00)
                    return true;
                if (entry[0] == 0xe5 ||
                    entry[11] == fat32_attribute_long_name ||
                    (entry[11] & 0x08) != 0 ||
...
static bool fat32_nth_cluster(const fat32_filesystem_t *filesystem,
...
static bool fat32_chain_length(const fat32_filesystem_t *filesystem,
...
static bool fat32_resize_chain(const fat32_filesystem_t *filesystem,
    ...
    ui32 last_cluster   = 0;
    if (!fat32_chain_length(filesystem, *first_cluster, &current_length,
                            &last_cluster))
        return false;
    ...
    ui32 keep_last = 0;
    if (!fat32_nth_cluster(filesystem, *first_cluster, cluster_count - 1,
                           &keep_last))
        return false;
...
static bool fat32_write_range(const fat32_filesystem_t *filesystem,
                              ui32 first_cluster, ui32 offset,
                              const ui8 *buffer, ui32 size, bool zero_fill) {
    if (size == 0)
        return true;
    if (!fat32_cluster_valid(filesystem, first_cluster)) {
        ;
    ...
    while (bytes_written < size) {
        ...
        while (sector_index < filesystem->sectors_per_cluster &&
            ...
            if (zero_fill) {
                ...
                copy_bytes(sector + sector_offset, buffer + bytes_written,
                           chunk);
            }
...
static bool fat32_write_dot_entries(const fat32_filesystem_t *filesystem,
                                    ui32 cluster, ui32 parent_cluster) {
    ui32 cluster_lba;
    if (!fat32_cluster_lba(filesystem, cluster, &cluster_lba))
        return false;
    ...
    ui8 dot_entry[32];
    ui8 dotdot_entry[32];
    fat32_build_entry(dot_entry, dot_name, fat32_attribute_directory, cluster,
                      0);
...
bool fat32_create_file(fat32_filesystem_t *filesystem, const char *path) {
    ...
    bool                   exists = false;
    if (!fat32_resolve_parent(filesystem, path, &parent_cluster, name,
                              &existing, &exists)) {
        return false;
    }
    if (exists) {
...
bool fat32_rename(fat32_filesystem_t *filesystem, const char *path,
    ...
    if (!fat32_resolve_parent(filesystem, path, &source_parent_cluster,
                              source_name, &source, &source_exists)) {
        return false;
    }
    if (!source_exists) {
        ;
    ...
    if (!fat32_resolve_parent(filesystem, new_path, &target_parent_cluster,
                              target_name, &target, &target_exists)) {
        return false;
    }
    if (target_exists) {
        ;
    ...
    bool source_is_directory =
        (source.entry[11] & fat32_attribute_directory) != 0;
