...
static ui32 read_pixel(ui32 x, ui32 y) {
    ...
    if (g_graphics.bits_per_pixel == 32) return *(volatile ui32 *) pixel;
...
void graphics_move_cursor(i32 delta_x, i32 delta_y) {
    if (cursor.visible) {
        for (ui32 y = 0; y < cursor_height; y++) {
            for (ui32 x = 0; x < cursor_width; x++) {
                if (cursor_shape[y][x] != '.') write_pixel(cursor.x + x, cursor.y + y, cursor.background[y][x]);
            }
        }
    } else {
    ...
    if (cursor.x > static_cast<i32>(g_graphics.width - cursor_width)) cursor.x = g_graphics.width - cursor_width;
    if (cursor.y > static_cast<i32>(g_graphics.height - cursor_height)) cursor.y = g_graphics.height - cursor_height;
