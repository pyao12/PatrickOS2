...
static bool is_letter(ui8 sc) { return (sc >= 0x10 && sc <= 0x19) || (sc >= 0x1E && sc <= 0x26) || (sc >= 0x2C && sc <= 0x32); }

static bool keyboard_shift;
static bool keyboard_capslock;
...
extern "C" void keyboard_interrupt() {
    ...
    if (scancode == 0x2A || scancode == 0x36) {
        ...
        if (is_letter(scancode)) {
            bool upper = keyboard_shift ^ keyboard_capslock;
            c          = upper ? sc1_shifted[scancode] : sc1_normal[scancode];
        } else {
            c = keyboard_shift ? sc1_shifted[scancode] : sc1_normal[scancode];
        }
        if (c != 0)
