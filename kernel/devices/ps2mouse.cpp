...
static bool wait_to_write() {
    for (ui32 attempt = 0; attempt < ps2_timeout; attempt++) {
        if (!(inb(ps2_status_port) & 0x02)) return true;
    }
    return false;
}
...
static bool wait_to_read(bool auxiliary) {
    for (ui32 attempt = 0; attempt < ps2_timeout; attempt++) {
        ...
        if ((status & 0x01) && static_cast<bool>(status & 0x20) == auxiliary) return true;
    }
    return false;
}

static bool write_controller(ui8 value) {
    if (!wait_to_write()) return false;
...
static bool write_mouse(ui8 value) {
    if (!write_controller(0xD4) || !write_data(value) || !wait_to_read(true)) return false;
    return inb(ps2_data_port) == 0xFA;
}

bool ps2mouse_init() {
    if (!write_controller(0xA8) || !write_controller(0x20) || !wait_to_read(false)) return false;

    ui8 configuration = inb(ps2_data_port);
    configuration &= static_cast<ui8>(~0x22);
    if (!write_controller(0x60) || !write_data(configuration)) return false;

    if (!write_mouse(0xF6) || !write_mouse(0xF4)) return false;
